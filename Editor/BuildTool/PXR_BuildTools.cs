...
public class PXR_BuildTools : EditorWindow
{
    ...
    [MenuItem("PXR_SDK/Build Tool/BuildAndRun")]
    static void BuildAndRun()
    {
        GetWindow(typeof(PXR_BuildTools));
        showCancel = false;
        buildFailed = false;
        totalBuildTime = 0;
        ...
        if (buildResult.summary.result == UnityEditor.Build.Reporting.BuildResult.Succeeded)
        {
            ...
            showCancel = true;
            gradlePath = PXR_ADBTool.GetGradlePath();
            jdkPath = PXR_ADBTool.GetJDKPath();
            androidSdkPath = PXR_ADBTool.GetAndroidSDKPath();
    ...
    public static bool DeployAPK()
    {
        ...
        var adbTool = new PXR_ADBTool(androidSdkPath);
    ...
    {
        var adbTool = new PXR_ADBTool(PXR_ADBTool.GetAndroidSDKPath());
        if (adbTool.isReady)
        {
